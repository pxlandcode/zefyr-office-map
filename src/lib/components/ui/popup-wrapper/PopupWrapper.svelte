...
{#if open}
	...
	>
		...
		<div
			...
			on:pointerdown|capture={reportInteraction}
			on:touchstart|capture={reportInteraction}
			on:wheel|capture={reportInteraction}
			on:scroll|capture={reportInteraction}
			on:focusin|capture={reportInteraction}
			on:keydown|capture={reportInteraction}
			style={modalStyle}
			role="dialog"
			aria-modal="true"
		...
		>
			<div class="modal-inner flex h-full flex-col">
				<div class="modal-header flex items-center justify-between gap-3 border-b-2 px-5 pb-2 pt-4">
					<div class="flex items-center gap-2">
						{#if icon}
							<div class="flex items-center justify-center w-7 h-7 rounded-full bg-text text-white">
								<Icon {icon} size="14px" />
							</div>
						{/if}
					...
					<div class="flex items-center gap-2">
						<div class="relative flex items-center justify-center">
							<div class="relative z-10">
								<IconButton on:click={onClose} size="18px" icon="Close" type="cancel" transparent />
							</div>
							<div
