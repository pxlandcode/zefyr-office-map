<script lang="ts">
	...
	export let selectedOption: OptionItem | null = null;
	...
	const wrapperClassByType = {
		...
		cancel: 'bg-gradient-to-r from-red to-red-dark'
	} as const;
...
<div
	class={`${wrapperClassByType[type]} p-[2px] rounded-lg w-full max-w-md`}
>
	<div class="flex gap-[2px] rounded-lg">
		{#each options as option, index}
			<button
				...
				class={`flex-1 h-[44px] text-center text-sm font-semibold transition-all duration-200 active:scale-90
